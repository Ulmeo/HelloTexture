...
{
	const UINT mipmapLevel = 8;
	const UINT rowPitch = TextureWidth * TexturePixelSize;
	const UINT cellPitch = rowPitch >> mipmapLevel;		// The width of a cell in the checkboard texture.
	const UINT cellHeight = TextureWidth >> mipmapLevel;	// The height of a cell in the checkerboard texture.
	...
	for (UINT n = 0; n < textureSize; n += TexturePixelSize)
	{
		...
		UINT xMipmap = i * cellPitch + cellPitch / 2;
		UINT yMipmap = j * cellHeight + cellHeight / 2;

		//get text
		pData[n] = xMipmap * 255 / rowPitch;	// R
		pData[n + 1] = yMipmap * 255 / TextureHeight;	// G
		pData[n + 2] = 255 - (pData[n] + pData[n+1]) / 2;	// B
		pData[n + 3] = 0xff;	// A
	}
